...
function setNoteAccordionOpen(id, open) {
...
function syncNoteAccordionHeader(entry) {
  ...
  const txt =
    (label?.textContent || label?.innerText || "").trim() || entry.fallback || "";
...
function renderSeatCell(r, col) {
  ...
  if (
    ...
    appendSeatIcon(
      ib,
      SEAT_ICONS.mealPending,
      L.lgServed || L.clearTray || "Served"
    );
  }
  if (seat?.served && seat.served.trayCleared) {
    appendSeatIcon(
      ib,
      SEAT_ICONS.trayCleared,
      L.lgclear || L.undoClearTray || "Cleared"
    );
  }
  if (seat.sleep) {
    appendSeatIcon(
      ib,
      SEAT_ICONS.sleep,
      L.lgSleep || L.mdSleepTxt || "Sleeping"
    );
  }
  if (seat.serveLaterAt) {
    appendSeatIcon(
      ib,
      SEAT_ICONS.later,
      L.lgLater || L.mdLater || "Serve later"
    );
  }
  d.appendChild(ib);
...
function seatWheelClampOffset(offset) {
  const min =
    seatWheelState.items.length > 0
      ? -((seatWheelState.items.length - 1) * seatWheelState.itemSize)
      : 0;
  const max = 0;
...
function seatWheelSetOffset(offset, animate = false) {
  const track = seatWheelState.track;
  if (!track) return;
  const nextOffset = seatWheelClampOffset(offset);
  seatWheelState.offset = nextOffset;
  if (!animate) track.style.transition = "none";
  else track.style.transition = "transform 0.18s ease";
  track.style.transform = `translateX(${nextOffset}px)`;
  if (animate) {
    const tidy = () => {
      track.style.transition = "none";
      track.removeEventListener("transitionend", tidy);
    };
    track.addEventListener("transitionend", tidy);
  }
}
function seatWheelSnapToIndex(index, triggerSeat = false) {
  if (!seatWheelState.items.length) return;
  const clamped = Math.max(
    0,
    Math.min(seatWheelState.items.length - 1, index)
  );
  seatWheelState.lastSnapIndex = clamped;
  seatWheelSetOffset(-(clamped * seatWheelState.itemSize), true);
  const active = seatWheelState.items[clamped];
  markSeatWheelSelection(active?.key || "");
  if (triggerSeat && active?.key && active.key !== modalSeat?.key) {
    focusSeatByKey(active.key);
  }
}
function seatWheelSnap(triggerSeat = false) {
  if (!seatWheelState.items.length) return;
  const approx = Math.round(
    Math.abs(seatWheelState.offset) / seatWheelState.itemSize
  );
  seatWheelSnapToIndex(approx, triggerSeat);
}
function handleSeatWheelPointerDown(ev) {
  if (!seatWheelState.container) return;
  if (ev.pointerType === "mouse" && ev.button !== 0) return;
  seatWheelState.dragging = true;
  seatWheelState.pointerId = ev.pointerId;
  seatWheelState.startX = ev.clientX;
  seatWheelState.startOffset = seatWheelState.offset;
  seatWheelState.container.setPointerCapture(ev.pointerId);
  ev.preventDefault();
}
function handleSeatWheelPointerMove(ev) {
  if (!seatWheelState.dragging || ev.pointerId !== seatWheelState.pointerId)
    return;
  const delta = ev.clientX - seatWheelState.startX;
  seatWheelSetOffset(seatWheelState.startOffset + delta, false);
}
function handleSeatWheelPointerUp(ev) {
  if (!seatWheelState.dragging || ev.pointerId !== seatWheelState.pointerId)
    return;
  seatWheelState.dragging = false;
  seatWheelState.pointerId = null;
  seatWheelState.container?.releasePointerCapture(ev.pointerId);
  seatWheelSnap(true);
}
function handleSeatWheelWheel(ev) {
  if (!seatWheelState.track) return;
  ev.preventDefault();
  const rawDelta =
    Math.abs(ev.deltaX) > Math.abs(ev.deltaY) ? ev.deltaX : ev.deltaY;
  seatWheelSetOffset(seatWheelState.offset - rawDelta, false);
  if (seatWheelState.snapTimer) clearTimeout(seatWheelState.snapTimer);
  seatWheelState.snapTimer = setTimeout(() => seatWheelSnap(true), 120);
}
function handleSeatWheelKey(ev) {
  if (!seatWheelState.items.length) return;
  if (ev.key !== "ArrowLeft" && ev.key !== "ArrowRight") return;
  ev.preventDefault();
  const delta = ev.key === "ArrowLeft" ? -1 : 1;
  const baseIndex =
    typeof seatWheelState.lastSnapIndex === "number"
      ? seatWheelState.lastSnapIndex
      : seatWheelState.items.findIndex((item) => item.key === modalSeat?.key);
  const nextIndex = Math.max(
    0,
    Math.min(seatWheelState.items.length - 1, (baseIndex || 0) + delta)
  );
  seatWheelSnapToIndex(nextIndex, true);
}
function ensureSeatWheelListeners() {
  const container = seatWheelState.container;
  if (!container || container.dataset.wheelReady === "1") return;
  container.dataset.wheelReady = "1";
  container.addEventListener("pointerdown", handleSeatWheelPointerDown);
  container.addEventListener("pointermove", handleSeatWheelPointerMove);
  container.addEventListener("pointerup", handleSeatWheelPointerUp);
  container.addEventListener("pointercancel", handleSeatWheelPointerUp);
  container.addEventListener("wheel", handleSeatWheelWheel, {
    passive: false,
  });
  container.addEventListener("keydown", handleSeatWheelKey);
  container.addEventListener("click", (ev) => {
    const item = ev.target.closest(".seat-wheel__item");
    if (!item) return;
    const idx = Number(item.dataset.index || "0");
    seatWheelSnapToIndex(idx, true);
  });
}
function markSeatWheelSelection(activeKey) {
  if (!seatWheelState.track) return;
  const buttons = seatWheelState.track.querySelectorAll(".seat-wheel__item");
  buttons.forEach((btn) => {
    const isActive = btn.dataset.seat === activeKey;
    btn.classList.toggle("is-active", isActive);
    btn.setAttribute("aria-selected", isActive ? "true" : "false");
  });
}
function seatWheelAlignToSeat(key) {
  if (!seatWheelState.items.length || !key) {
    seatWheelSetOffset(0);
    markSeatWheelSelection("");
    return;
  }
  const idx = seatWheelState.items.findIndex((item) => item.key === key);
  if (idx === -1) {
    seatWheelSetOffset(0);
    markSeatWheelSelection("");
    return;
  }
  seatWheelState.lastSnapIndex = idx;
  seatWheelState.offset = -(idx * seatWheelState.itemSize);
  seatWheelSetOffset(seatWheelState.offset);
  markSeatWheelSelection(key);
}
function renderSeatWheel(items) {
  const carousel = document.getElementById("passengerCarousel");
  if (!carousel) return;
  seatWheelState.container = carousel;
  seatWheelState.items = items;
  carousel.classList.add("seat-wheel");
  ...
  if (!items.length) {
    ...
    seatWheelState.track = null;
    return;
  }
  const mask = document.createElement("div");
  mask.className = "seat-wheel__mask";
  const track = document.createElement("div");
  track.className = "seat-wheel__track";
  items.forEach((item, index) => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "seat-wheel__item";
    btn.dataset.index = String(index);
    btn.dataset.seat = item.key;
    btn.setAttribute("role", "option");
    btn.setAttribute(
      "aria-selected",
      item.key === modalSeat?.key ? "true" : "false"
    );
    if (!item.data?.occupied && item.key !== modalSeat?.key) {
      btn.classList.add("is-empty");
    }
    if (item.data?.served?.meal) {
      btn.classList.add("is-served");
    }
    ...
    label.className = "seat-wheel__label";
    label.textContent = item.key;
    btn.appendChild(label);
    track.appendChild(btn);
  });
  mask.appendChild(track);
  carousel.appendChild(mask);
  const focusRing = document.createElement("div");
  focusRing.className = "seat-wheel__focus";
  carousel.appendChild(focusRing);
  seatWheelState.track = track;
  ...
  if (firstItem) {
    ...
    if (rect.width > 0) {
      seatWheelState.itemSize = rect.width + gap;
    }
  }
  ensureSeatWheelListeners();
  const targetKey = modalSeat?.key || seatWheelState.items[0]?.key || "";
  seatWheelAlignToSeat(targetKey);
}
