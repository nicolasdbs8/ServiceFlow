...
function ensureMenuDefaults() {
    if (!store.menu)
        store.menu = {};
    const m = store.menu;
    if (m.mode !== "manual" && m.mode !== "auto")
        m.mode = MENU_DEFAULTS.mode;
    ...
    if (m.rotation === undefined)
        m.rotation = null;
    if (typeof m.viandeLabel !== "string")
        m.viandeLabel = "";
    if (typeof m.vegeLabel !== "string")
        m.vegeLabel = "";
    ...
    if (typeof m.autoViandeLabel !== "string")
        m.autoViandeLabel = "";
    if (typeof m.autoVegeLabel !== "string")
        m.autoVegeLabel = "";
    if (typeof m.autoStatus !== "string")
        m.autoStatus = "";
    if (typeof m.autoNote !== "string")
        m.autoNote = "";
}
const SEAT_ICONS = {
    infant: "\uD83D\uDC76", // 👶
...
const SLEEP_BUTTON_IDS = [
    "sleepMealBtn",
    "sleepAperoBtn",
    "sleepTCBtn",
    "sleepModalBtn",
];
const NOTE_ACCORDION_SECTIONS = [
    {
        id: "aperoNotesBlock",
        labelId: "mdAperoNotes",
        fallback: "Drink / aperitif notes",
    },
    {
        id: "tcNotesBlock",
        labelId: "mdTCNotes",
        fallback: "Tea & coffee notes",
    },
    {
        id: "mealDrinkNotesBlock",
        labelId: "mdMealDrinkNotes",
        fallback: "Meal drink notes",
    },
];
const noteAccordionRegistry = new Map();
const BULK_OCC_CONFIRM_THRESHOLD = 20;
let bulkOccupyMode = false;
const bulkOccupySelection = new Set();
function ensureNotesAccordions() {
    NOTE_ACCORDION_SECTIONS.forEach((cfg) => {
        const block = document.getElementById(cfg.id);
        if (!block)
            return;
        if (noteAccordionRegistry.has(cfg.id)) {
            const existing = noteAccordionRegistry.get(cfg.id);
            bindNoteAccordionLabel(existing);
            syncNoteAccordionHeader(existing);
            hideNoteAccordionLabel(existing);
            return;
        }
        const existingChildren = Array.from(block.childNodes);
        const content = document.createElement("div");
        content.className = "notes-accordion__content";
        existingChildren.forEach((child) => {
            content.appendChild(child);
        });
        const header = document.createElement("div");
        header.className = "accordion__header notes-accordion__header";
        header.setAttribute("role", "button");
        header.setAttribute("tabindex", "0");
        const headerText = document.createElement("span");
        headerText.className = "notes-accordion__title";
        header.appendChild(headerText);
        const body = document.createElement("div");
        body.className = "accordion__body notes-accordion__body";
        ...
        block.classList.add("accordion", "notes-accordion");
        block.dataset.notesAccordion = "1";
        ...
        noteAccordionRegistry.set(cfg.id, entry);
        header.addEventListener("click", () => toggleNoteAccordion(entry));
        header.addEventListener("keydown", (ev) => {
            if (ev.key === "Enter" || ev.key === " ") {
                ev.preventDefault();
                toggleNoteAccordion(entry);
            }
        });
        setNoteAccordionState(entry, entry.defaultOpen);
        bindNoteAccordionLabel(entry);
        hideNoteAccordionLabel(entry);
    });
}
function bindNoteAccordionLabel(entry) {
    if (!entry || entry.labelObserver || !entry.labelId)
        return;
    ...
    entry.labelObserver = observer;
}
function syncNoteAccordionHeader(entry) {
    if (!entry)
        return;
    const label = entry.labelId ? document.getElementById(entry.labelId) : null;
    const txt = (label?.textContent || label?.innerText || "").trim() ||
        entry.fallback ||
        "";
    entry.headerText.textContent = txt;
}
function setNoteAccordionState(entry, open) {
    if (!entry)
        return;
    entry.section.dataset.open = open ? "true" : "false";
    entry.header.setAttribute("aria-expanded", open ? "true" : "false");
    entry.body.hidden = !open;
}
function toggleNoteAccordion(entry) {
    if (!entry)
        return;
    const isOpen = entry.section.dataset.open === "true";
    setNoteAccordionState(entry, !isOpen);
}
function hideNoteAccordionLabel(entry) {
    if (!entry)
        return;
    const label = entry.block.querySelector("label");
    if (label) {
        label.classList.add("sr-only");
        label.setAttribute("aria-hidden", "true");
    }
}
function syncNoteAccordionsByPhase(phase) {
    const setOpen = (id, open) => {
        const entry = noteAccordionRegistry.get(id);
        if (entry)
            setNoteAccordionState(entry, open);
    };
    if (phase === "aperitif") {
        setOpen("aperoNotesBlock", true);
        setOpen("tcNotesBlock", false);
        setOpen("mealDrinkNotesBlock", false);
    }
    else if (phase === "tc") {
        setOpen("aperoNotesBlock", false);
        setOpen("tcNotesBlock", true);
        setOpen("mealDrinkNotesBlock", false);
    }
    else if (phase === "repas") {
        setOpen("aperoNotesBlock", false);
...
function ensureSeatShape(s) {
    ...
    if (s.spml === undefined)
        s.spml = "";
    if (s.preLabel === undefined)
        s.preLabel = "";
    if (s.normalMeal === undefined)
        s.normalMeal = "";
    if (s.aperoNotes === undefined)
        s.aperoNotes = "";
    if (s.tcNotes === undefined)
        s.tcNotes = "";
    if (!s.alloc || typeof s.alloc !== "object")
        s.alloc = { normalKey: null, spmlCode: null, preLabel: null };
    if (!s.served)
        s.served = { aperitif: null, meal: null };
    if (typeof s.served.trayUsed !== "boolean")
        s.served.trayUsed = false;
    if (s.serveLaterAt === undefined)
        s.serveLaterAt = null;
    if (typeof s.served.tc === "undefined")
        s.served.tc = null; // horodatage th/caf
    if (typeof s.served.trayCleared === "undefined")
        s.served.trayCleared = false; // plateau dbarrass
    if (!s.apDrink)
        s.apDrink = {
    ...
    if (s.eatWith === undefined)
        s.eatWith = ""; // cl sige partenaire (ex: "4C") ou ""
    return s;
}
...
(function attachHeaderButtons() {
    ...
    document.getElementById("importFile")?.addEventListener("change", (e) => {
        ...
        if (window.__importingJSON)
            return;
        window.__importingJSON = true;
        const releaseImportLock = () => setTimeout(() => {
    ...
    document.getElementById("exportPNG")?.addEventListener("click", () => {
        try {
            if (typeof exportSeatmapPNG === "function")
                exportSeatmapPNG();
            else
                console.warn("exportSeatmapPNG() non dfinie");
        }
        catch (e) { }
    });
})();
...
function sumSPML() {
    return Object.values(store.inventory.spml || {}).reduce((a, b) => a + (b || 0), 0);
}
function sumPRE() {
    return Object.values(store.inventory.pre || {}).reduce((a, b) => a + (b || 0), 0);
}
function rightCols() {
...
function currentSeatFilter() {
    const active = document.querySelector("#filterTabs .subtab.active");
    return active?.dataset.filter || "all";
}
function setActiveFilterTab(filter) {
    document
        .querySelectorAll("#filterTabs .subtab")
        .forEach((btn) => {
...
function setActivePhaseTab(phase) {
    document
        .querySelectorAll("#phaseChips .tab")
        .forEach((btn) => {
...
function updateModalStatusBadge() {
    ...
    if (!meta)
        return;
    meta.innerHTML = "";
    const select = document.getElementById("m_status");
    ...
    pill.className =
        STATUS_PILL_CLASS[key] ||
            STATUS_PILL_CLASS.OTHER;
    pill.textContent = status;
    meta.appendChild(pill);
}
function syncLanguageButtons() {
    const select = document.getElementById("m_lang");
...
function syncLayoutButtons() {
    const hidden = document.getElementById("layoutSelect");
...
function syncLaterButtons() {
    const select = document.getElementById("m_later");
    const value = select?.value || "";
    const occupied = isModalSeatOccupied();
    document.querySelectorAll(".later-btn").forEach((btn) => {
    ...
    const timeInput = document.getElementById("m_later_time");
    const shouldShowCustom = value === "custom" && occupied;
    if (container)
        container.classList.toggle("active", shouldShowCustom);
    if (timeInput) {
...
function isModalSeatOccupied() {
    const checkbox = document.getElementById("m_occ_chk");
    return !!checkbox?.checked;
}
function updateModalOccupancyControls() {
    const occupied = isModalSeatOccupied();
    document
        .querySelectorAll(".language-btn")
        .forEach((btn) => {
    ...
    const langSelect = document.getElementById("m_lang");
    if (langSelect)
        langSelect.disabled = !occupied;
    document
        .querySelectorAll('input[name="m_type"]')
        .forEach((input) => {
        input.disabled = !occupied;
    });
    const statusSelect = document.getElementById("m_status");
    if (statusSelect)
        statusSelect.disabled = !occupied;
    const eatWithSelect = document.getElementById("m_eatWith");
    if (eatWithSelect)
        eatWithSelect.disabled = !occupied;
...
const legendButton = document.getElementById("legendToggle");
const legendPopover = document.getElementById("legendPopover");
const legendSeatmapContainer = document.querySelector(".seatmap");
function positionLegendPopover() {
    if (!legendButton || !legendPopover || !legendSeatmapContainer)
        return;
...
function setLegendPopoverState(open) {
    if (!legendButton || !legendPopover)
        return;
...
function isLegendPopoverOpen() {
    return legendPopover?.dataset.open === "true";
}
function refreshBulkOccupyHighlights() {
...
function updateBulkOccupyUI() {
    const L = I18N[store.config.lang || "EN"] || I18N.EN;
    const toggle = document.getElementById("bulkOccupyToggle");
    const apply = document.getElementById("bulkOccupyApply");
    const clearBtn = document.getElementById("bulkOccupyClear");
    const status = document.getElementById("bulkOccupyStatus");
    const grid = document.getElementById("seatgrid");
    const count = bulkOccupySelection.size;
    if (grid)
        grid.classList.toggle("bulk-occupy-mode", !!bulkOccupyMode);
    if (toggle) {
    ...
    if (apply) {
        ...
        apply.textContent = (tpl || "").replace("{n}", String(Math.max(1, count || 1)));
...
function toggleBulkSeatSelection(key) {
    if (!bulkOccupyMode)
        return;
    ...
    if (seat.occupied) {
        if (status)
            status.textContent = L.bulkOccBlocked || "Seat already occupied";
        return;
    }
    if (bulkOccupySelection.has(key))
        bulkOccupySelection.delete(key);
    else
        bulkOccupySelection.add(key);
    updateBulkOccupyUI();
}
...
function applyBulkOccupySelection() {
    if (!bulkOccupyMode)
        return;
    const count = bulkOccupySelection.size;
    if (count === 0) {
    ...
    if (count >= BULK_OCC_CONFIRM_THRESHOLD) {
        const L = I18N[store.config.lang || "EN"] || I18N.EN;
        const msg = (L.bulkOccConfirm || "Confirm marking {n} seats as occupied?").replace("{n}", String(count));
    ...
    for (const k of keys) {
        ...
        seat.occupied = true;
        store.seats[k] = seat;
        const el = document.querySelector(`#seatgrid .seat[data-seat-key="${k}"]`);
        if (el)
...
function handleSeatGridClick(key) {
    if (!bulkOccupyMode)
        return false;
...
function updateFlightDatePretty() {
    ...
    if (span && input)
        span.textContent = fmtDateLocalized(input.value);
}
function currentFlightDateISO() {
    const hidden = document.getElementById("flightDateISO");
    if (hidden && hidden.value)
        return String(hidden.value).trim();
    const visible = document.getElementById("flightDate");
...
function updateMenuPreviewUI(result) {
    ensureMenuDefaults();
    const L = I18N[store.config.lang || "EN"] || I18N.EN;
    const hasDirection = store.menu.direction === "outbound" || store.menu.direction === "inbound";
    const hasService = store.menu.serviceType === "day" || store.menu.serviceType === "breakfast";
    const hasDate = !!currentFlightDateISO();
    const preview = document.getElementById("menuPreview");
    if (preview)
        preview.style.display = hasDirection && hasService ? "flex" : "none";
    ...
    if (opt1Title)
        opt1Title.textContent = L.lblViande || "Option 1";
    if (opt2Title)
        opt2Title.textContent = L.lblVege || "Option 2";
...
function syncMenuUI() {
    ...
    document.querySelectorAll("[data-menu-direction]").forEach((btn) => {
        const active = btn.dataset.menuDirection === store.menu.direction;
        btn.classList.toggle("active", active);
        btn.setAttribute("aria-pressed", active ? "true" : "false");
    });
    document.querySelectorAll("[data-menu-service]").forEach((btn) => {
        const active = btn.dataset.menuService === store.menu.serviceType;
        btn.classList.toggle("active", active);
        btn.setAttribute("aria-pressed", active ? "true" : "false");
    });
...
function updateNormalInfoPopup() {
    ...
    const btn = document.getElementById("normalInfoBtn");
    ...
    const autoOpt1 = (store.menu.viandeLabel ||
        store.menu.autoViandeLabel ||
        "").trim();
    const autoOpt2 = (store.menu.vegeLabel ||
        store.menu.autoVegeLabel ||
        "").trim();
    const hasLabels = mode === "manual"
...
function initMenuControls() {
    ...
    document.querySelectorAll("[data-menu-mode]").forEach((btn) => {
        tap(btn, () => {
            const mode = btn.dataset.menuMode || "auto";
            setMenuMode(mode);
        });
    });
    document.querySelectorAll("[data-menu-direction]").forEach((btn) => {
        tap(btn, () => {
            const dir = btn.dataset.menuDirection === "inbound"
                ? "inbound"
                : "outbound";
    ...
    document.querySelectorAll("[data-menu-service]").forEach((btn) => {
        btn.addEventListener("click", () => {
            const svc = btn.dataset.menuService === "breakfast"
                ? "breakfast"
                : "day";
    ...
    const handleNightstop = () => {
        if (!nightstop || nightstop.disabled)
            return;
    ...
    const syncManualInputs = () => {
        ensureMenuDefaults();
        if (lv)
            store.menu.manualViandeLabel = (lv.value || "").trim();
        if (lg)
            store.menu.manualVegeLabel = (lg.value || "").trim();
        if (store.menu.mode === "manual") {
            store.menu.viandeLabel = store.menu.manualViandeLabel || "";
            store.menu.vegeLabel = store.menu.manualVegeLabel || "";
        }
...
function applyI18n() {
    ...
    if (Array.isArray(typeof I18N_BINDINGS !== "undefined" ? I18N_BINDINGS : [])) {
        for (let i = 0; i < I18N_BINDINGS.length; i++) {
            ...
            if (_ew && !_ew.title)
                _ew.title = L.mdEatWithPh || "e.g. 12B";
            const el = sel && (sel.startsWith("#") || sel.startsWith("."))
                ? document.querySelector(sel)
                : document.getElementById(sel);
            if (el && L[key] !== undefined)
    ...
    (function setBigDrinkCaps() {
        ...
        const capMap = {
            ...
            drink_red: () => L.tcWines && L.tcRed ? `${L.tcWines}  ${L.tcRed}` : "Red wine",
            drink_white: () => L.tcWines && L.tcWhite ? `${L.tcWines}  ${L.tcWhite}` : "White wine",
    ...
    // -- Localise les tooltips des grosses catgories (grille boissons)
    {
        ...
        setT('#alcoRow .pill.big[data-sub="vin_rouge"]', L.tcWines && L.tcRed ? `${L.tcWines}  ${L.tcRed}` : "Red wine");
        setT('#alcoRow .pill.big[data-sub="vin_blanc"]', L.tcWines && L.tcWhite ? `${L.tcWines}  ${L.tcWhite}` : "White wine");
    ...
    if (eatw)
        eatw.placeholder = L.mdEatWithPh || "e.g. 12B";
    // Placeholders dynamiques
    const lv = document.getElementById("labelViande");
    const lg = document.getElementById("labelVege");
    if (lv)
        lv.placeholder = L.meatPlaceholder || "e.g., Beef in red-wine sauce";
    if (lg)
        lg.placeholder = L.vegPlaceholder || "e.g., Vegetarian lasagna";
    const f = document.getElementById("filterTabs");
    if (f) {
        const label = lang === "FR" ? "Filtrer" : lang === "DE" ? "Filtern" : "Filter";
    ...
    if (typeof updateBulkOccupyUI === "function")
        updateBulkOccupyUI();
    ...
    if (preSelect) {
        const choosePreorder = L.preAddSelectPlaceholder ||
            (lang === "FR"
                ? "Choisir un code"
                : lang === "DE"
                    ? "Code auswaehlen"
    ...
    if (pre)
        pre.placeholder = L.preAddPlaceholder || "e.g., Gluten-free pasta";
    const laterCustomButton = document.querySelector(".later-btn[data-value='custom']");
    if (laterCustomButton) {
        const label = (L.mdLaterCustomBtn || "Custom").trim() || "Custom";
        laterCustomButton.textContent = label;
    }
    const resetButton = document.getElementById("resetFlight");
    if (resetButton) {
    ...
    if (legalEl)
        legalEl.textContent = L.rights || "All rights reserved";
    ...
    if (typeof updateSeatmapTitle === "function")
        updateSeatmapTitle();
    ...
    if (fd) {
        if (store.config.lang === "FR")
            fd.placeholder = "JJ/MM/AAAA";
        else if (store.config.lang === "DE")
            fd.placeholder = "TT.MM.JJJJ";
        else
            fd.placeholder = "MM, DD, YYYY";
    }
    updateFlightDatePretty();
    updateHistoryTitle();
...
function renderSeatCell(r, col) {
    ...
    dot.className = "dot";
    if (seat.status === "FTL")
        dot.classList.add("ftl");
    else if (seat.status === "SEN")
        dot.classList.add("sen");
    else if (seat.status === "HON")
        dot.classList.add("hon");
    else if (seat.status === "VIP")
        dot.classList.add("vip");
    else if (seat.status === "FCL")
        dot.classList.add("fcl");
    else if (seat.status === "PAD")
        dot.classList.add("pad");
    ...
    if (seat?.served && seat.served.trayCleared) {
        // The cleared badge is intentionally hidden (pax no longer needs a pill)
    }
    ...
    addClickAndTouchListener(d, (e) => {
        ...
        if (handleSeatGridClick(seatKey))
            return;
        onSeatClick(seatKey);
    });
    ...
    if (flt === "toServe") {
        ...
        const phaseKey = store.phase === "aperitif"
            ? "aperitif"
            : store.phase === "tc"
                ? "tc"
                : "meal"; // "repas"  "meal"
...
function refreshBadges() {
    ...
    const minusLabel = lang === "FR" ? "Diminuer" : lang === "DE" ? "Reduzieren" : "Decrease";
    const plusLabel = lang === "FR" ? "Augmenter" : lang === "DE" ? "Erh�hen" : "Increase";
    ...
    if (addSel) {
        ...
        const addTxt = lang === "FR" ? "Choisir" : lang === "DE" ? "Waehlen" : "Choose";
    ...
    if (preAddSelect) {
        ...
        const addTxt = (I18N[lang]?.preAddSelectPlaceholder || "").trim() ||
            (lang === "FR"
                ? "Choisir un code"
                : lang === "DE"
                    ? "Code auswaehlen"
...
function adjInv(group, key, delta) {
    if (group === "spml") {
        store.inventory.spml[key] = Math.max(0, (store.inventory.spml[key] || 0) + delta);
    }
    else if (group === "pre") {
        store.inventory.pre[key] = Math.max(0, (store.inventory.pre[key] || 0) + delta);
    }
    else {
...
function updateServeMealButtonState() {
    ...
    const hiddenNormal = document.getElementById("m_normalMealValue");
    const chosen = (hiddenNormal?.value || d.normalMeal || "").trim();
    if (hiddenNormal)
        hiddenNormal.value = chosen;
    const hiddenSpml = document.getElementById("m_spmlValue");
    if (hiddenSpml)
        hiddenSpml.value = d.spml || "";
    const preSel = $("#m_pre");
    if (preSel)
...
function buildPassengerCarousel() {
    ...
    const prevBtn = document.getElementById("seatNavPrev");
    const nextBtn = document.getElementById("seatNavNext");
    const currentLabel = document.getElementById("seatNavCurrent");
    if (currentLabel)
        currentLabel.textContent = currentKey || "--";
...
function mealOptionLabel(type, L) {
    ...
    if (type === "plateau")
        return L.optNormalTray || "Tray (no casserole)";
    return type || "";
}
function renderNormalMealCards(seatData) {
    const container = document.getElementById("m_normalMeal");
    const hidden = document.getElementById("m_normalMealValue");
    if (!container)
        return;
    const L = I18N[store.config.lang || "EN"];
...
function renderSpmlCards(seatData) {
    ...
    const hidden = document.getElementById("m_spmlValue");
    if (!row || !container)
        return;
...
function handleMealTouchEnd(ev) {
    ...
    if (mealGestureCtx.mode === "actions") {
        if (absDx > absDy && absDx > MEAL_SWIPE_THRESHOLD) {
            const btn = dx > 0
                ? document.getElementById("serveMeal")
                : document.getElementById("serveMealNone");
            if (btn && !btn.disabled)
                btn.click();
        }
...
function openSeatModal(row, col) {
    ...
    if (moveBtn) {
        ...
        moveBtn.setAttribute("aria-disabled", (!modalSeat.data.occupied).toString());
    }
    updateSleepButtons();
    $("#modalTitle").textContent = I18N[store.config.lang || "EN"].seat;
    ...
    if (_lblNotes)
        _lblNotes.textContent = L.mdNotes || "Notes";
    ...
    if (laterCustom)
        laterCustom.classList.remove("active");
    const laterTimeInput = document.getElementById("m_later_time");
    if (laterTimeInput) {
    ...
    const occ = $("#m_occ_chk");
    if (occ)
        occ.checked = !!modalSeat.data.occupied;
    ...
    if (sl)
        sl.checked = !!modalSeat.data.sleep;
    updateSleepButtons();
    ...
    if (store.phase === "aperitif") {
        ...
        if (mhTC)
            mhTC.textContent = "";
        if (mhMeal)
            mhMeal.textContent = ml; // on garde lheure du plat servi
    ...
    function arrangeModalByPhase(phase, L) {
        ...
        const mealContent = meal?.querySelector(".modal-card__content");
        const tc = document.getElementById("tcBlock");
        const aperoBlock = document.getElementById("aperoBlock");
        const tcServeBlock = document.getElementById("tcServeBlock");
        let rightNotes = document.getElementById("rightNotes");
        ensureNotesAccordions();
        syncNoteAccordionsByPhase(phase);
        if (!rightNotes) {
        ...
        if (tc) {
            ...
            if (apNotes && apNotes.parentElement !== tc)
                tc.appendChild(apNotes);
            if (tcNotes && tcNotes.parentElement !== tc)
                tc.appendChild(tcNotes);
        }
        if (aperoBlock && aperoBlock.parentElement !== tc)
            tc?.appendChild(aperoBlock);
        if (tcServeBlock && tcServeBlock.parentElement !== tc)
            tc?.appendChild(tcServeBlock);
        const mdn = document.getElementById("mealDrinkNotesBlock");
        const serveRow = document.getElementById("mealDrinkServeRow");
        if (mealContent) {
            if (mdn && mdn.parentElement !== mealContent)
                mealContent.appendChild(mdn);
            if (serveRow && serveRow.parentElement !== mealContent)
                mealContent.appendChild(serveRow);
        }
        if (mdn)
            mdn.style.display = "none";
        if (serveRow)
            serveRow.style.display = "none";
        if (aperoBlock)
            aperoBlock.style.display = "none";
        if (tcServeBlock)
            tcServeBlock.style.display = "none";
        if (meal && meal.parentElement !== rightCol)
            rightCol?.appendChild(meal);
        if (tc && tc.parentElement !== rightCol)
        ...
        if (laterRow)
            laterRow.style.display = "";
        if (phase === "fiche") {
            // FICHE CLIENT : uniquement la colonne gauche (passager)
            if (modal)
                modal.classList.add("mono");
            if (rightCol)
                rightCol.style.display = "none";
            if (laterRow)
                laterRow.style.display = "none"; // on cache "Serve later"
        ...
        if (phase === "aperitif") {
            ...
            if (modal)
                modal.classList.remove("mono");
            if (tc && leftCol)
                leftCol.appendChild(tc);
            if (meal)
                meal.style.display = "none"; // pas de meal ici
            if (tc) {
                const apNotes = document.getElementById("aperoNotesBlock");
                const tcNotes = document.getElementById("tcNotesBlock");
                if (tcNotes)
                    tcNotes.style.display = "none";
                if (apNotes) {
                ...
                if (aperoBlock) {
                    aperoBlock.style.display = "flex";
                    rightNotes?.appendChild(aperoBlock);
                    if (rightNotes)
                        rightNotes.style.display = "block";
                }
                // titre = Apritif
                ...
                if (title)
                    title.textContent = L.tcTitle || "Aperitif";
            }
        ...
        if (phase === "tc") {
            ...
            if (tc) {
                ...
                if (tcServeBlock) {
                    tcServeBlock.style.display = "flex";
                    rightNotes?.appendChild(tcServeBlock);
                    if (rightNotes)
                        rightNotes.style.display = "block";
                }
                // titre = Th & Caf
                const title = document.getElementById("tcTitle");
                if (title)
                    title.textContent = L.chips_tc || "Tea & Coffee";
            }
        ...
        if (phase === "repas") {
            ...
            if (tc && leftCol) {
                ...
                if (tcNotes)
                    tcNotes.style.display = "none";
                // >>> AFFICHER + DPLACER le bloc notes Meal Drink sous le bloc repas
                const mdn = document.getElementById("mealDrinkNotesBlock");
                if (mdn) {
                    ...
                    if (lbl)
                        lbl.textContent = L.mealDrinkNotes || "Meal drink notes";
                }
                ...
                const title = document.getElementById("tcTitle");
                if (title)
                    title.textContent = L.mealDrinkTitle || "Meal drinks";
            }
            if (meal && rightCol) {
            ...
            //  gauche on ne montre QUE le bloc boissons
            reactivateDrinkGrid();
            updateServeDrinkButtons();
            initDrinkGrid(); //  AJOUT
            ensureDrinkGridActive();
...
function tc_updateAlcoolUI() {
    ...
    if (v) {
        if (catSel)
            catSel.value = v; // biere | vin | champagne | digestif
        tc_showSub(v);
    }
    else {
        if (catSel)
...
function tc_updateChaudUI() {
    ...
    if (v) {
        if (catSel)
            catSel.value = v; // cafe | deca | the | chocolat
...
function writeTcToUI(d) {
    ...
    if (bOpt)
        bOpt.style.display = ap.digestif === "baileys" ? "flex" : "none";
...
function persistModal() {
    ...
    if (typeRadio)
        d.type = typeRadio.value; // "normal" | "child" | "infant"
    const sl = document.getElementById("m_sleep_chk");
    if (sl)
        d.sleep = !!sl.checked; // sinon: on ne touche pas d.sleep
    ...
    if (ew) {
        ...
        const detachPartner = (key, expected) => {
            if (!key)
                return;
...
function quickToggleSleep() {
    ...
    if (chk)
        chk.checked = modalSeat.data.sleep;
...
function updateSleepButtons() {
    ...
    const L = I18N[store.config.lang || "EN"] || I18N.EN;
    const sleepLabel = L.mdSleepTxt || "Sleep";
    SLEEP_BUTTON_IDS.forEach((id) => {
        const btn = document.getElementById(id);
        if (!btn)
            return;
        btn.textContent = SEAT_ICONS.sleep;
        btn.title = sleepLabel;
        btn.setAttribute("aria-label", sleepLabel);
    });
    const modalBtn = document.getElementById("sleepModalBtn");
    if (modalBtn) {
...
function performMoveOrSwap(fromKey, toKey) {
    ...
    if (typeof renderSeatmap === "function")
        renderSeatmap();
    if (typeof renderServiceFlow === "function")
        renderServiceFlow();
}
...
function summarizeApDrink(ap, L, lang) {
    ...
    if (ap) {
        // Bire
        if (ap.beer === "calvinus_blanche")
            ap.beer = "calvinus";
        if (ap.beer && ap.beer.startsWith("leermond"))
            ap.beer = "leermond";
        // Vin
    ...
    const addLemon = (f) => {
        if (f)
            P.push(L.tcLemon || "Lemon");
    };
    // Helpers lait/sucre (pour chaud)
    const milkLabel = (v) => {
        if (v === "creme")
            return L.tcMilkCreme;
        if (v === "avoine")
            return L.tcMilkAvoine;
        if (v === "lait")
            return L.tcMilkLait;
        if (v === "none")
            return L.tcMilkNone;
    ...
    const sweetLabel = (v) => {
        if (v === "sucre")
            return L.tcSweetSugar;
        if (v === "succedane")
            return L.tcSweetSub;
        if (v === "none")
            return L.tcSweetNone;
        return "";
    };
    switch (ap.cat) {
        // ===== CHAUD =====
        ...
        case "the":
            ...
            if (ap.theType === "hotwater") {
                ...
                if (ap.theLemon)
                    P.push(L.tcLemon || "Citron");
                break; // on sort ici pour ne pas afficher "th"
            }
            P.push(L.tcCatThe);
            if (ap.theType === "english")
                P.push(L.tcTeaEnglish);
            else if (ap.theType === "vert")
                P.push(L.tcTeaVert);
            else if (ap.theType === "menthe")
                P.push(L.tcTeaMenthe);
        ...
        case "digestif":
            P.push(L.tcCatDig);
            if (ap.digestif === "whisky_jw")
                P.push(L.tcDigJW);
            else if (ap.digestif === "whisky_jb")
                P.push(L.tcDigJB);
            else if (ap.digestif === "cognac")
                P.push(L.tcDigCognac);
            else if (ap.digestif === "baileys")
                P.push(L.tcDigBaileys);
            // Compat historique si jamais whiskyStyle existe encore,
            // sinon on utilise la case  cocher gnrique "Ice"
            if (ap.whiskyStyle === "pur")
                P.push(L.tcPur);
            else if (ap.whiskyStyle === "rocks")
                P.push(L.tcRocks);
            else
                addIce(!!ap.digIce);
            break;
        case "biere": {
            ...
            if (ap.beer && mapBeer[ap.beer])
                P.push(mapBeer[ap.beer]);
            else if (ap.beer)
                P.push(ap.beer);
            break;
        }
        ...
        case "cocktail": {
            ...
            const c = ap.cocktail || "";
            if (c === "gin_tonic")
                P.push(L.tcGinTonic || "Gin Tonic");
            else if (c === "cuba_libre")
                P.push(L.tcCubaLibre || "Cuba Libre");
            else if (c === "screwdriver")
                P.push(L.tcScrewDriver || "Screw Driver");
            else if (c === "bloody_mary") {
                P.push(L.tcBloodyMary || "Bloody Mary");
                if (ap.virginMary)
                    P.push(L.tcVirginMary || "Virgin Mary");
                if (ap.cocktailSP)
                    P.push([L.tcSalt || "Salt", L.tcPepper || "Pepper"].join(" + "));
            }
            else if (c === "campari") {
                P.push("Campari");
                if (ap.campariMix === "orange")
                    P.push(L.tcJuiceOrange || "Orange");
                else if (ap.campariMix === "soda")
                    P.push(L.tcSoda || "Soda");
            }
            if (ap.cocktailIce)
                P.push(L.tcIce || "Ice");
            if (ap.cocktailLemon)
        ...
        case "soft":
            if (ap.softType === "eau") {
                P.push(L.tcLblWater);
                if (ap.waterType === "plate")
                    P.push(L.tcWaterStill);
                else if (ap.waterType === "gazeuse")
                    P.push(L.tcWaterSpark);
                addIce(!!ap.softIce);
                ...
                if (ap.juiceType === "pomme") {
                    if (ap.juiceApfelschorle)
                        P.push("Apfelschorle");
                    else
                        P.push(L.tcJuiceApple || "Apple");
                }
                else if (ap.juiceType === "orange")
                    P.push(L.tcJuiceOrange);
                else if (ap.juiceType === "tomate")
                    P.push(L.tcJuiceTomato);
                addIce(!!ap.juiceIce);
                ...
                P.push("Coca-Cola", ap.cocaType === "zero"
                    ? "Zero"
                    : ap.cocaType === "normal"
                        ? "Classic"
                        : "");
...
document.getElementById("serveAperitif")?.addEventListener("click", () => {
    ...
    if (!d.served.aperitif)
        d.served.aperitif = Date.now();
...
document.getElementById("serveTC")?.addEventListener("click", () => {
    ...
    if (store.phase === "tc" && !d.served.tc)
        d.served.tc = Date.now();
...
document.getElementById("serveMealDrink")?.addEventListener("click", () => {
    ...
    if (typeof readTcFromUI === "function")
        readTcFromUI(d);
    ...
    if (typeof resetDrinkUI === "function")
        resetDrinkUI(true);
    if (typeof updateServeDrinkButtons === "function")
        updateServeDrinkButtons(); //  AJOUT
    if (typeof reopenSameSeat === "function")
        reopenSameSeat();
});
...
$("#serveMeal").addEventListener("click", () => {
    ...
    if (wasNone)
        wasServed = false; // <- si c'tait Rien, on traite comme pas encore servi
    ...
    const needsTray = (m, seat) => m === "plateau" ||
        m === "viande" ||
        m === "vege" ||
        !!seat.spml ||
        !!seat.preLabel;
...
$("#m_later").addEventListener("change", () => {
    if (typeof syncLaterButtons === "function")
        syncLaterButtons();
});
function scheduleReminderFor(seat, minutes, atTimeStr = null) {
    ...
    store.reminders = (store.reminders || []).filter((r) => r.key !== modalSeat.key);
...
$("#m_later_time").addEventListener("change", () => {
    if (typeof setLaterButtonState === "function")
        setLaterButtonState();
});
$("#m_later_apply").addEventListener("click", () => {
    if (!modalSeat)
        return;
    if (!isModalSeatOccupied())
        return;
    const L = I18N[store.config.lang || "EN"];
...
document.addEventListener("click", (event) => {
    ...
    if (container.contains(target))
        return;
    if (target.classList?.contains("later-btn") &&
        target.dataset.value === "custom")
        return;
    if (target.closest(".form-field--actions"))
        return;
    if (target.id === "m_later_apply" || target.id === "m_later_cancel")
        return;
    const select = document.getElementById("m_later");
    ...
    if (timeInput?.value)
        return;
    const seat = modalSeat?.data;
    if (seat?.serveLaterAt && seat.serveLaterAt > Date.now())
        return;
    select.value = "";
    select.dispatchEvent(new Event("change", { bubbles: true }));
});
...
["m_status", "m_lang", "m_notes", "m_aperoNotes", "m_eatWith"].forEach((id) => {
    const el = document.getElementById(id);
    if (el)
        el.addEventListener("change", persistModal);
    const elTcNotes = document.getElementById("m_tcNotes");
    if (elTcNotes)
        elTcNotes.addEventListener("change", persistModal);
});
// nouveaux contrles
...
function renderReminders() {
    const lang = store.config.lang || "EN";
    const cancelTxt = lang === "FR" ? "Annuler" : lang === "DE" ? "Lschen" : "Cancel";
    ...
    for (const r of arr) {
        ...
        del.addEventListener("click", () => {
            const idx = store.reminders.findIndex((x) => x.key === r.key && x.at === r.at);
            if (idx >= 0)
                store.reminders.splice(idx, 1);
            if (store.seats[r.key])
                store.seats[r.key].serveLaterAt = null;
            save();
            renderReminders();
            renderSeatmap();
            renderServiceFlow();
...
function colExists(c) {
    if (c === "B")
        return store.config.layout === "A320"; // B absent sur A220
...
function seatCategoryRank(seat) {
    ...
    if (seat.status === "SEN")
        return 2;
    if (seat.status === "PAD")
        return 4; // PAD en dernier pour la prise de commande
    return 3;
}
...
function computeServiceFlow() {
    ...
    for (let r = 1; r <= rows; r++) {
        for (const c of COL_ORDER) {
            ...
            if (!seat.occupied)
                continue;
            if (seat.served[phaseKey])
                continue;
            if (seat.serveLaterAt && seat.serveLaterAt > now)
                continue;
...
const FLOW_CONFIRM_ARMED = new Set();
function attachFlowTapConfirm(row, seatKey, phase, action) {
    let armed = false;
    let armTimer = null;
    const confirmEl = row.querySelector(".flow-serve-confirm") ||
        (() => {
            const el = document.createElement("div");
...
function renderServiceFlow() {
    ...
    if (laterSection)
        laterSection.style.display = isFiche ? "none" : "";
            ...
            if (list.length === 0) {
                ...
                for (const ev of list) {
                    ...
                    if (typeof parseSeatKey === "function") {
                        ...
                        if (r && c && typeof seatObj === "function")
                            d = seatObj(r, c);
                    }
                    const tags = [];
                    if (d) {
                    ...
                    const label = typeof summarizeApDrink === "function"
...
document.querySelectorAll("[data-inv]").forEach((btn) => btn.addEventListener("click", () => {
...
document
    ...
    ?.addEventListener("change", (e) => e.target.blur());
document.querySelectorAll("#filterTabs .subtab[data-filter]").forEach((btn) => btn.addEventListener("click", () => {
    if (btn.classList.contains("active"))
        return;
    const filter = btn.dataset.filter || "all";
    setActiveFilterTab(filter);
    if (isLegendPopoverOpen())
        setLegendPopoverState(false);
    renderSeatmap();
    renderServiceFlow(); //  AJOUT
}));
setActiveFilterTab(currentSeatFilter());
const bulkToggleBtn = document.getElementById("bulkOccupyToggle");
const bulkApplyBtn = document.getElementById("bulkOccupyApply");
const bulkClearBtn = document.getElementById("bulkOccupyClear");
if (bulkToggleBtn) {
...
if (legendPopover && legendButton && legendSeatmapContainer) {
    ...
    document.addEventListener("click", (event) => {
        ...
        if (!target)
            return;
        if (legendPopover.contains(target) || legendButton.contains(target))
            return;
        setLegendPopoverState(false);
    });
    ...
    document.addEventListener("keydown", (event) => {
        if (event.key !== "Escape" || !isLegendPopoverOpen())
            return;
        setLegendPopoverState(false);
        legendButton.focus({ preventScroll: true });
    });
}
document.querySelectorAll("#phaseChips .tab").forEach((btn) => btn.addEventListener("click", () => {
    if (btn.classList.contains("active"))
        return;
...
$("#exportPNG").addEventListener("click", () => {
    ...
    const name = (store.title.flightNo || "vol") +
        "_" +
...
function drawSeatPNG(ctx, r, col, x, y, w, h) {
    ...
    if (seat.status !== "none") {
        if (seat.status === "FTL")
            ...
            ctx.fillStyle = "#9b5cff"; // violet
        else if (seat.status === "FCL")
            ctx.fillStyle = "#ff5757"; // rouge
        else if (seat.status === "PAD")
            ctx.fillStyle = "#39d98a"; // vert
        else
            ctx.fillStyle = "#666";
        ctx.beginPath();
...
if (resetBtn) {
    ...
    resetBtn.addEventListener("click", () => {
        const lang = store.config.lang || "EN";
        const L = I18N[lang] || I18N.EN;
        const confirmLabel = (L.resetConfirm ?? L.reset ?? "Confirm").trim() || "Confirm";
...
function migrateHistoryToEvents() {
    ...
    for (const raw of store.history) {
        ...
        let m;
        if ((m = s.match(/^(Apritif servi |Aperitif served at|Aperitif serviert bei)\s+([0-9]+[A-F])/))) {
            out.push({ ts: Date.now(), type: "apServed", seat: m[2] });
            continue;
        }
        if ((m = s.match(/^(Apritif annul pour|Aperitif canceled for|Aperitif storniert fr)\s+([0-9]+[A-F])/))) {
            out.push({ ts: Date.now(), type: "apCanceled", seat: m[2] });
            continue;
        }
        if ((m = s.match(/^(Plat servi |Main served at|Hauptgang serviert bei)\s+([0-9]+[A-F])(?:\s+\s+(.+))?/))) {
            out.push({
                ts: Date.now(),
                type: "mealServed",
                seat: m[2],
        ...
        if ((m = s.match(/^(Service annul pour|Service canceled for|Service storniert fr)\s+([0-9]+[A-F])/))) {
...
(function init() {
    ...
    if (layoutSel2)
        layoutSel2.value = store.config.layout || "A220";
    syncLayoutButtons();
    const langSel2 = document.getElementById("langSelect");
    if (langSel2)
        langSel2.value = store.config.lang || "EN";
    applyI18n();
    renderHistory();
    ...
    renderSeatmap();
    if (disp)
        disp.textContent = store.config.rowsBiz || 0;
    ensureMenuDefaults();
...
(function replaceCocktailChangeHandler() {
    ...
    document.addEventListener("click", (ev) => {
        ...
        const tip = document.getElementById("tc_tip");
        if (wrap && tip && !wrap.contains(ev.target))
            tip.style.display = "none";
    });
...
function drinkEmoji(ap) {
    ...
    if (ap.cat === "soft") {
        ...
        if (ap.softType === "jus")
            return "";
        if (ap.softType === "sprite")
            return "";
        if (ap.softType === "tonic")
            return "";
    }
    // Alcool
    if (ap.cat === "champagne")
...
function mealDrinkLineHTML(seatKey) {
    ...
    if (!ev)
        return ""; // rien  afficher si pas encore servie
    const L = I18N[store.config.lang || "EN"];
    let label = "";
    if (ev.ap) {
...
export function syncUIToStoreForExport() {
    ...
    const fd = document.getElementById("flightDate");
    if (!store.title)
        store.title = { flightNo: "", date: "" };
    if (fn)
        store.title.flightNo = (fn.value || "").trim();
    if (fdISO && fdISO.value)
        store.title.date = fdISO.value;
    else if (fd)
        store.title.date = fd.value;
...
export function initFlightFormsLegacy() {
    if (typeof initFlightDatePicker === "function")
        initFlightDatePicker();
    attachHeaderButtons?.();
    initLangButtons?.();
    applyTheme?.();
}
export function initMealSectionLegacy() {
    initializeDrinkMenu();
    initDrinkGrid();
    reactivateDrinkGrid();
    updateServeDrinkButtons();
    ...
    if (mealBlock && !mealBlock.dataset.touchReady) {
        ...
        mealBlock.addEventListener("touchcancel", () => {
            mealGestureCtx = null;
        }, { passive: true });
        mealBlock.dataset.touchReady = "1";
    }
}
export function initPassengersSectionLegacy() {
    setTooltips?.();
}
export function initAirportSectionLegacy() {
...
export { store, SPML_CODES, PREORDER_CODES, ensureSeatShape, seatObj, storageKey, sumSPML, sumPRE, rightCols, };
