...
export async function setupPersistence(): Promise<void> {
  ...
  try {
    for (let i = 0; i < originalStorage.length; i += 1) {
      ...
      if (!value) continue;
      if (key === LAST_KEY) {
        setLastFlightKey(value).catch((error) =>
          console.warn("[persistence] setLastFlightKey failed during migration:", error),
        );
        continue;
      }
      if (isFlightKey(key)) {
        try {
          ...
          saveFlight(key, payload).catch((error) =>
            console.warn("[persistence] saveFlight failed during migration:", error),
          );
        } catch (error) {
  ...
  Storage.prototype.setItem = function patchedSetItem(key: string, value: string): void {
    ...
    if (key === LAST_KEY) {
      setLastFlightKey(value).catch((error) =>
        console.warn("[persistence] setLastFlightKey failed:", error),
      );
      return;
    }
    ...
    if (isFlightKey(key)) {
      try {
        const payload = JSON.parse(value);
        saveFlight(key, payload).catch((error) =>
          console.warn("[persistence] saveFlight failed:", error),
        );
      } catch (error) {
  ...
  Storage.prototype.removeItem = function patchedRemoveItem(key: string): void {
    ...
    if (isFlightKey(key)) {
      deleteFlight(key).catch((error) =>
        console.warn("[persistence] deleteFlight failed:", error),
      );
    }
  };
}
 ...
 */
export async function loadPersistedSnapshot<T = unknown>(): Promise<{ key: string; snapshot: T } | null> {
  const key = (await getLastFlightKey()) ?? readLegacyLastKey();
  if (!key) return null;

  const snapshot = (await loadFlight<T>(key)) ?? readLegacySnapshot<T>(key);
...
function readLegacyLastKey(): string | null {
  try {
...
function readLegacySnapshot<T>(key: string): T | null {
  try {
...
function isFlightKey(key: string): boolean {
