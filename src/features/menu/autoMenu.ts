...
export type MenuDirection = "outbound" | "inbound";
export type MenuServiceType = "day" | "breakfast";
export type MenuBreakfastType = "standard" | "nightstop";
type LangKey = keyof typeof I18N;

export interface MenuLookupInput {
  dateISO?: string | null;
...
export interface AutoMenuResult {
  rotation: number | null;
  option1: string;
  option2: string;
...
interface MenuRotationEntry {
  rotation: number;
  direction: MenuDirection | "both";
  serviceType: MenuServiceType;
  breakfastType?: MenuBreakfastType;
  label1: string;
  label2: string;
  ...
  note?: string;
  noteKey?: string;
  dataStatus?: "ok" | "placeholder";
}

const DEFAULT_LANG: LangKey = "EN";

function getBundle(lang?: string | null) {
  const key = (lang || DEFAULT_LANG) as LangKey;
  return (I18N as any)[key] || (I18N as any)[DEFAULT_LANG] || {};
}

function translateField(bundle: any, key?: string, fallback = ""): string {
...
const PLACEHOLDER_MENU: MenuRotationEntry[] = [
  {
    rotation: 1,
    direction: "both",
...
const ROTATION_DATES: { rotation: number; dates: ReadonlySet<string> }[] = [
  {
    rotation: 1,
...
export function computeMenuRotationFromDate(dateISO?: string | null): number | null {
  if (!dateISO) return null;
  const iso = normalizeDateString(dateISO);
  if (!iso) return null;
  for (const entry of ROTATION_DATES) {
    if (entry.dates.has(iso)) return entry.rotation;
  }
  return null;
}
...
export function lookupAutoMenuLabels(input: MenuLookupInput): AutoMenuResult {
  const bundle = getBundle(input.lang);
  const direction: MenuDirection = input.direction || "outbound";
  const serviceType: MenuServiceType = input.serviceType || "day";
  const breakfastType: MenuBreakfastType = input.breakfastType || "standard";
  const rotation = input.rotation ?? computeMenuRotationFromDate(input.dateISO);

  const matches = PLACEHOLDER_MENU.filter((entry) => {
    if (rotation && entry.rotation !== rotation) return false;
    if (entry.serviceType !== serviceType) return false;
    if (entry.direction !== "both" && entry.direction !== direction) return false;
    if (entry.serviceType === "breakfast") {
      const type = entry.breakfastType || "standard";
      if (type !== breakfastType) return false;
    }
    return true;
  });

  const picked =
    matches.find((m) => m.dataStatus === "ok") ||
  ...
  return {
    ...
    note: rotation
      ...
      : translateField(
          ...
          "Selected date is outside defined rotations."
        ),
  };
}
