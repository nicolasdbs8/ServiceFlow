...
function syncNoteAccordionHeader(entry) {
  ...
  const txt =
    (label?.textContent || label?.innerText || "").trim() ||
    entry.fallback ||
    "";
...
function syncNoteAccordionsByPhase(phase: string) {
...
function renderSeatCell(r, col) {
  ...
  if (!seat?.served?.trayCleared) {
    ...
    if (tagText) {
      ...
      if (vertical) tag.classList.add("vertical");
      d.appendChild(tag);
    }
  }
  ...
  for (const it of list) {
    ...
    switch (it.type) {
      ...
      case "apOrdered": {
        ...
        const label = summarizeApDrink(it.ap, L, store.config.lang) || "";
        const notes = it.notes ? `  ${it.notes}` : "";
        msg = `${L.histApOrdered || "Aperitif ordered at"} ${seat}${
          label ? " - " + label : ""
        }${notes}`;
        break;
      }
...
function buildPassengerCarousel() {
  ...
  if (nav) {
    nav.setAttribute(
      "data-empty",
      passengerCarouselOrder.length ? "false" : "true"
    );
  }
...
function mealOptionLabel(type, L) {
  if (type === "viande") return L.optNormalMeat || "Option 1";
  if (type === "vege") return L.optNormalVeg || "Option 2";
...
function renderSpmlCards(seatData) {
  ...
  if (useDropdown) {
    ...
    available.forEach(({ code, qty, isChosen }) => {
      ...
      if (qty <= 0 && !isChosen) opt.disabled = true;
...
function populateModalOptions() {
  ...
  for (const [label, qty] of Object.entries(store.inventory.pre)) {
    ...
    if (inStock || isChosen) {
      const o = document.createElement("option");
      o.value = label;
      o.textContent = `${label} (${Math.max(0, qty as number)})`;
...
function openSeatModal(row, col) {
  ...
  function arrangeModalByPhase(phase, L) {
    ...
    if (serveRow) serveRow.style.display = "none";
    if (aperoBlock) aperoBlock.style.display = "none";
    if (tcServeBlock) tcServeBlock.style.display = "none";
    ...
    if (phase === "repas") {
      ...
      if (tc && leftCol) {
        ...
        if (mdn) {
          ...
          if (mealContent) mealContent.appendChild(mdn);
        ...
        if (serveRow) {
          ...
          if (mealContent) mealContent.appendChild(serveRow);
        }
...
function writeTcToUI(d) {
  ...
  if (ap.cat === "soft") {
    ...
    ["champagne", "vin_rouge", "vin_blanc", "biere", "cocktail", "digestif"].includes(
      ap.cat || ""
    )
  ) {
    drinkSel.cat = "alcool";
...
function updateModalPhaseNav() {
  ...
  const setTabLabel = (btn: HTMLElement | null, key: string) => {
    ...
    if (normalized) span.textContent = normalized;
  };
...
function mealChoiceTagHTML(d) {
  ...
  if (d.preLabel) {
    const shortLabel =
      typeof d.preLabel === "string" && d.preLabel.length > 10
...
function mealHistoryLabel(d: any, L: any) {
  if (!d) return "";
  if (d.spml) return d.spml;
  if (d.preLabel) {
    const lab = String(d.preLabel || "").trim();
    return lab.length > 10 ? `${lab.slice(0, 10)}...` : lab;
  }
  const nm = (d.normalMeal || "").trim();
  if (nm === "viande") return L?.optNormalMeat || "Option 1";
  if (nm === "vege") return L?.optNormalVeg || "Option 2";
  if (nm === "plateau") return L?.trayShort || "Tray";
  return "";
}

function attachFlowSwipeHandler(
  row: HTMLElement,
  seatKey: string,
  phase: string
) {
...
function markMealServedFromFlow(seatKey: string, seat: any) {
  const L = I18N[store.config.lang || "EN"];
  if (!seat || !seat.occupied) return;
  if (!seat.served) seat.served = {};
  if (seat.served.meal) return;
...
function markTrayClearedFromFlow(seatKey: string, seat: any) {
  const L = I18N[store.config.lang || "EN"];
  if (!seat || !seat.occupied) return;
  if (!seat.served || !seat.served.meal || !seat.served.trayUsed) return;
  if (seat.served.trayCleared) return;
