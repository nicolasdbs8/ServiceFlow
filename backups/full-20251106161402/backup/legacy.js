...
function emojiForBig(cat, sub) {
  return (DRINK_EMOJI_BIG[cat] && DRINK_EMOJI_BIG[cat][sub]) || "";
}
function emojiForOpt(key) {
  return DRINK_EMOJI_OPT[key] || "";
}
function emojiForFlag(flag) {
  return DRINK_EMOJI_FLAG[flag] || "";
}
function applyDrinkEmojis(scope) {
...
// === NOUVEAU : dlgation unique DRINK GRID (pointer events)
(function bindDrinkGrid() {
  ...
  document.addEventListener(
    ...
    function (ev) {
      ...
      if (!t) return;
      // === 1) CLIC SUR UNE GROSSE PILL (catgorie)
      if (t.classList.contains("big")) {
        ...
        // [FIX immdiat] activer le bon bouton pour les 1er-niveau autoriss
        (function () {
          const L1_OK = new Set([
            "cafe",
            "deca",
          ...
          if (store.phase === "aperitif")
            btn = document.getElementById("serveAperitif");
          else if (store.phase === "tc")
            btn = document.getElementById("serveTC");
          else if (store.phase === "repas")
            btn = document.getElementById("serveMealDrink");
          if (btn) btn.disabled = false;
        })();
        if (typeof focusDrinkGroup === "function")
          focusDrinkGroup(drinkSel.cat);
        if (typeof renderOptionsFor === "function")
          renderOptionsFor(drinkSel.cat, drinkSel.sub);
        if (typeof updateServeDrinkButtons === "function")
          updateServeDrinkButtons();
        return;
      }
      // === 2) OPTIONS EXCLUSIVES (data-opt="cl:valeur")
      if (t.dataset.opt) {
        const [key, val] = t.dataset.opt.split(":");
        const scope = t.closest(".pill-subrows") || grid;
        // mmoriser l'tat avant de vider le groupe
        const wasActive = t.classList.contains("active");
        // exclusivit dans le mme groupe
        scope.querySelectorAll(`.pill[data-opt^="${key}:"]`).forEach((x) => {
          x.classList.remove("active");
          x.setAttribute("aria-pressed", "false");
        });
        // toggle correct : si on reclique le mme => on dslectionne
        if (wasActive) {
        ...
        // rgles d'affichage spciales
        if (key === "juiceType") {
          // sel+poivre visibles seulement pour
          document
            .querySelectorAll("#softOpts .jt-extra")
            .forEach((x) => (x.style.display = val === "tomate" ? "" : "none"));
        }
        if (key === "cocktail") {
          const showC = val === "campari";
          const showB = val === "bloody_mary";
          const cr = document.getElementById("campariRow"),
            ct = document.getElementById("campariTitle");
          const br = document.getElementById("bmRow"),
            bt = document.getElementById("bmTitle");
          const spBtn = document.querySelector(
            '#alcoOpts .pill.toggle[data-flag="cocktailSP"]'
          );
          if (cr && ct) {
            cr.style.display = showC ? "" : "none";
            ct.style.display = showC ? "" : "none";
          }
          if (br && bt) {
            br.style.display = showB ? "" : "none";
            bt.style.display = showB ? "" : "none";
          }
      ...
      if (t.classList.contains("toggle") && t.dataset.flag) {
        ...
        // combos
        if (key === "juiceSP") {
          drinkSel.juiceSalt = next;
          drinkSel.juicePepper = next;
        }
        if (key === "cocktailSP") {
          drinkSel.cocktailSalt = next;
          drinkSel.cocktailPepper = next;
        }
        if (typeof updateServeDrinkButtons === "function")
          updateServeDrinkButtons();
        return;
    ...
    true
  ); // capture pour passer avant d'ventuels stopPropagation
})();
// Remplit les lgendes sous chaque emoji depuis le title (fallback immdiat)
(function initPillCaptions() {
  document.querySelectorAll("#drinkGrid .pill-wrap").forEach((wrap) => {
    ...
    if (!btn || !cap) return;
    // Si on a une cl i18n plus tard, elle remplacera ce texte
    cap.textContent =
...
function L() {
  return I18N[store.config.lang || "EN"] || I18N.EN;
}
function renderOptionsFor(cat, sub) {
  const tip = document.getElementById("drinkTip");
  if (tip) tip.style.display = "none";
  const L = (k, d) =>
    (I18N[store.config.lang || "EN"] || I18N.EN)[k] || d || "";
  const on = (sel, fn) => {
    document.querySelectorAll(sel).forEach(fn);
  };
  // --- HOT (caf, dca, th, chocolat) ---
  if (cat === "chaud") {
    const box = document.getElementById("hotOpts");
    if (!box) return;
    box.innerHTML = "";
    if (sub === "cafe" || sub === "deca") {
      box.innerHTML = `
  <div class="opt-title"> ${L("tcMilk", "Milk/Cream")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="milk:creme"  title="${L(
        "tcMilkCreme",
        "Cream"
      )}" aria-label="${L("tcMilkCreme", "Cream")}">${emojiForOpt(
        "milk:creme"
      )}</button>
      <div class="pill-cap">${L("tcMilkCreme", "Cream")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="milk:avoine" title="${L(
        "tcMilkAvoine",
        "Oat milk"
      )}" aria-label="${L("tcMilkAvoine", "Oat milk")}">${emojiForOpt(
        "milk:avoine"
      )}</button>
      <div class="pill-cap">${L("tcMilkAvoine", "Oat milk")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="milk:lait"   title="${L(
        "tcMilkLait",
        "Milk"
      )}" aria-label="${L("tcMilkLait", "Milk")}">${emojiForOpt(
        "milk:lait"
      )}</button>
      <div class="pill-cap">${L("tcMilkLait", "Milk")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="milk:none"   title="${L(
        "tcMilkNone",
        "None"
      )}" aria-label="${L("tcMilkNone", "None")}">${emojiForOpt(
        "milk:none"
      )}</button>
  ...
  <div class="opt-title"> ${L("tcSweet", "Sweetener")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="sweet:sucre"       title="${L(
        "tcSweetSugar",
        "Sugar"
      )}" aria-label="${L("tcSweetSugar", "Sugar")}">${emojiForOpt(
        "sweet:sucre"
      )}</button>
      <div class="pill-cap">${L("tcSweetSugar", "Sugar")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="sweet:succedane"   title="${L(
        "tcSweetSub",
        "Substitute"
      )}" aria-label="${L("tcSweetSub", "Substitute")}">${emojiForOpt(
        "sweet:succedane"
      )}</button>
      <div class="pill-cap">${L("tcSweetSub", "Substitute")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="sweet:none"        title="${L(
        "tcSweetNone",
        "None"
      )}" aria-label="${L("tcSweetNone", "None")}">${emojiForOpt(
        "sweet:none"
      )}</button>
      ...
      box.innerHTML = `
  <div class="opt-title"> ${L("tcCatThe", "Tea")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="theType:english"  title="${L(
        "tcTeaEnglish",
        "English Breakfast"
      )}" aria-label="${L("tcTeaEnglish", "English Breakfast")}">${emojiForOpt(
        "theType:english"
      )}</button>
      <div class="pill-cap">${L("tcTeaEnglish", "English Breakfast")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="theType:vert"     title="${L(
        "tcTeaVert",
        "Green"
      )}" aria-label="${L("tcTeaVert", "Green")}">${emojiForOpt(
        "theType:vert"
      )}</button>
      <div class="pill-cap">${L("tcTeaVert", "Green")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="theType:menthe"   title="${L(
        "tcTeaMenthe",
        "Mint"
      )}" aria-label="${L("tcTeaMenthe", "Mint")}">${emojiForOpt(
        "theType:menthe"
      )}</button>
      <div class="pill-cap">${L("tcTeaMenthe", "Mint")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="theType:hotwater" title="${L(
        "tcHotWater",
        "Hot water"
      )}" aria-label="${L("tcHotWater", "Hot water")}">${emojiForOpt(
        "theType:hotwater"
      )}</button>
  ...
  <div class="opt-title"> ${L("tcSweet", "Sugar")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="sweet:sucre"     title="${L(
        "tcSweetSugar",
  ...
  <div class="opt-title"> ${L("tcLemon", "Lemon")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="theLemon" title="${L(
        "tcLemon",
        "Lemon"
      )}" aria-label="${L("tcLemon", "Lemon")}">${emojiForFlag(
        "theLemon"
      )}</button>
      ...
      box.innerHTML = `<div class="opt-title"> ${L(
        "tcCatChoco",
        "Caotina"
      )}</div>`;
    }
  ...
  // --- SOFT (eau/cola/jus/sprite/tonic) ---
    ...
    const box = document.getElementById("softOpts");
    if (!box) return;
    box.innerHTML = "";
    if (sub === "eau") {
      box.innerHTML = `
        <div class="opt-title"> ${L("tcWater", "Water")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="waterType:plate"   title="${L(
        "tcWaterStill",
        "Still"
      )}"      aria-label="${L("tcWaterStill", "Still")}">${emojiForOpt(
        "waterType:plate"
      )}</button>
      <div class="pill-cap">${L("tcWaterStill", "Still")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="waterType:gazeuse" title="${L(
        "tcWaterSpark",
        "Sparkling"
      )}"  aria-label="${L("tcWaterSpark", "Sparkling")}">${emojiForOpt(
        "waterType:gazeuse"
      )}</button>
  ...
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="softIce"   title="${L(
        "tcIce",
        "Ice"
      )}"    aria-label="${L("tcIce", "Ice")}">${emojiForFlag(
        "softIce"
      )}</button>
      <div class="pill-cap">${L("tcIce", "Ice")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="softLemon" title="${L(
        "tcLemon",
        "Lemon"
      )}" aria-label="${L("tcLemon", "Lemon")}">${emojiForFlag(
        "softLemon"
      ...
      box.innerHTML = `
<div class="opt-title"> ${L("tcCola", "Cola")}</div>
<div class="pill-row">
  <div class="pill-wrap">
    <button class="pill" data-opt="cocaType:normal" title="${L(
      "tcCocaClassic",
      "Classic"
    )}" aria-label="${L("tcCocaClassic", "Classic")}">${emojiForOpt(
        "cocaType:normal"
      )}</button>
    <div class="pill-cap">${L("tcCocaClassic", "Classic")}</div>
  </div>
  <div class="pill-wrap">
    <button class="pill" data-opt="cocaType:zero"   title="${L(
      "tcCocaZero",
      "Zero"
    )}"       aria-label="${L("tcCocaZero", "Zero")}">${emojiForOpt(
        "cocaType:zero"
      )}</button>
...
<div class="pill-row">
  <div class="pill-wrap">
    <button class="pill toggle" data-flag="softIce"   title="${L(
      "tcIce",
      "Ice"
    )}"    aria-label="${L("tcIce", "Ice")}">${emojiForFlag("softIce")}</button>
    <div class="pill-cap">${L("tcIce", "Ice")}</div>
  </div>
  <div class="pill-wrap">
    <button class="pill toggle" data-flag="softLemon" title="${L(
    ...
    <div class="pill-cap">${L("tcLemon", "Lemon")}</div>
      ...
      box.innerHTML = `
  <div class="opt-title"> ${L("tcSprite", "Sprite")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="spriteIce" title="${L(
        "tcIce",
        "Ice"
      )}" aria-label="${L("tcIce", "Ice")}">${emojiForFlag(
        "spriteIce"
    ...
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="spriteLemon" title="${L(
        "tcLemon",
        "Lemon"
      )}" aria-label="${L("tcLemon", "Lemon")}">${emojiForFlag(
        "spriteLemon"
      ...
      box.innerHTML = `
  <div class="opt-title"> ${L("tcTonic", "Tonic")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="tonicIce" title="${L(
        "tcIce",
        "Ice"
      )}" aria-label="${L("tcIce", "Ice")}">${emojiForFlag("tonicIce")}</button>
      <div class="pill-cap">${L("tcIce", "Ice")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="tonicLemon" title="${L(
        "tcLemon",
        "Lemon"
      )}" aria-label="${L("tcLemon", "Lemon")}">${emojiForFlag(
        "tonicLemon"
      ...
      box.innerHTML = `
  <div class="opt-title"> ${L("tcJuice", "Juice")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="juiceType:orange" title="${L(
        "tcJuiceOrange",
        "Orange"
      )}" aria-label="${L("tcJuiceOrange", "Orange")}">${emojiForOpt(
        "juiceType:orange"
      )}</button>
      <div class="pill-cap">${L("tcJuiceOrange", "Orange")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="juiceType:pomme"  title="${L(
        "tcJuiceApple",
        "Apple"
      )}"   aria-label="${L("tcJuiceApple", "Apple")}">${emojiForOpt(
        "juiceType:pomme"
      )}</button>
      <div class="pill-cap">${L("tcJuiceApple", "Apple")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="juiceType:tomate" title="${L(
        "tcJuiceTomato",
        "Tomato"
      )}" aria-label="${L("tcJuiceTomato", "Tomato")}">${emojiForOpt(
        "juiceType:tomate"
      )}</button>
  ...
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="juiceIce"    title="${L(
        "tcIce",
        "Ice"
      )}"     aria-label="${L("tcIce", "Ice")}">${emojiForFlag(
        "juiceIce"
    ...
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="juiceLemon"  title="${L(
        "tcLemon",
        "Lemon"
      )}" aria-label="${L("tcLemon", "Lemon")}">${emojiForFlag(
        "juiceLemon"
    ...
    <div class="pill-wrap jt-extra" style="display:none">
      <button class="pill toggle" data-flag="juiceSP"
        ...
        aria-label="${L("tcSalt", "Salt")} + ${L(
        "tcPepper",
        "Pepper"
      )}">${emojiForFlag("juiceSP")}</button>
      <div class="pill-cap">${L("tcSalt", "Salt")} + ${L(
        "tcPepper",
        "Pepper"
      )}</div>
    </div>
  ...
  <div class="opt-title" id="apfelTitle" style="display:none">${L(
    "tcApfelschorle",
    "Apfelschorle"
  )}</div>
  <div class="pill-row" id="apfelRow" style="display:none">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="juiceApfelschorle" title="${L(
        "tcApfelschorle",
        "Apfelschorle"
      )}" aria-label="${L("tcApfelschorle", "Apfelschorle")}">${emojiForFlag(
        "juiceApfelschorle"
      )}</button>
      <div class="pill-cap">${L("tcApfelschorle", "Apfelschorle")}</div>
    </div>
  </div>`;
      // montrer sel/poivre seulement pour
      on('#softOpts .pill[data-opt^="juiceType"]', (b) =>
        b.addEventListener("click", () => {
          ...
          if (apfRow && apfTit) {
            apfRow.style.display = onApple ? "" : "none";
            apfTit.style.display = onApple ? "" : "none";
          }
        })
      );
    }
  ...
  // --- ALCOOL (bire, vins, champagne, digestif, cocktail) ---
  else if (cat === "alcool") {
    const box = document.getElementById("alcoOpts");
    if (!box) return;
    box.innerHTML = "";
    if (sub === "biere") {
      box.innerHTML = `
  ...
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="beer:quoellfrisch" title="${L(
        "tcBeerQ",
        "Qullfrisch Lager"
      )}" aria-label="${L("tcBeerQ", "Qullfrisch Lager")}">${emojiForOpt(
        "beer:quoellfrisch"
      )}</button>
      <div class="pill-cap">${L("tcBeerQ", "Qullfrisch Lager")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="beer:calvinus"     title="${L(
        "tcBeerC",
        "Calvinus Blanche"
      )}"  aria-label="${L("tcBeerC", "Calvinus Blanche")}">${emojiForOpt(
        "beer:calvinus"
      )}</button>
      <div class="pill-cap">${L("tcBeerC", "Calvinus Blanche")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="beer:leermond"     title="${L(
        "tcBeerL",
        "Leermond (0%)"
      )}"     aria-label="${L("tcBeerL", "Leermond (0%)")}">${emojiForOpt(
        "beer:leermond"
      )}</button>
      ...
      box.innerHTML = `
  <div class="opt-title"> ${L("tcWines", "Wines")}  ${L("tcRed", "Red")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="vinRouge:suisse"   title="${L(
        "tcCH",
        "Swiss"
      )}" aria-label="${L("tcCH", "Swiss")}">${emojiForOpt(
        "vinRouge:suisse"
      )}</button>
      <div class="pill-cap">${L("tcCH", "Swiss")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="vinRouge:etranger" title="${L(
        "tcForeign",
        "Foreign"
      )}" aria-label="${L("tcForeign", "Foreign")}">${emojiForOpt(
        "vinRouge:etranger"
      )}</button>
      ...
      box.innerHTML = `
  <div class="opt-title"> ${L("tcWines", "Wines")}  ${L(
        "tcWhite",
        "White"
      )}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="vinBlanc:suisse"   title="${L(
        "tcCH",
        "Swiss"
      )}"     aria-label="${L("tcCH", "Swiss")}">${emojiForOpt(
        "vinBlanc:suisse"
    ...
    <div class="pill-wrap">
      <button class="pill" data-opt="vinBlanc:etranger" title="${L(
        "tcForeign",
        "Foreign"
      )}" aria-label="${L("tcForeign", "Foreign")}">${emojiForOpt(
        "vinBlanc:etranger"
      ...
      box.innerHTML = `
  <div class="opt-title"> ${L("tcChampagne", "Champagne")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="champIce"    title="${L(
        "tcIce",
        "Ice"
      )}" aria-label="${L("tcIce", "Ice")}">${emojiForFlag("champIce")}</button>
      <div class="pill-cap">${L("tcIce", "Ice")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="champMimosa" title="Mimosa" aria-label="Mimosa">${emojiForFlag(
        "champMimosa"
      )}</button>
  ...
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="digestif:whisky_jw" title="${L(
        "tcDigJW",
        "Whisky  Johnnie Walker"
      )}" aria-label="${L("tcDigJW", "Whisky  Johnnie Walker")}">${emojiForOpt(
        "digestif:whisky_jw"
      )}</button>
      <div class="pill-cap">${L("tcDigJW", "Johnnie Walker")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="digestif:whisky_jb" title="${L(
        "tcDigJB",
        "Whisky  Jim Beam (Bourbon)"
      )}" aria-label="${L(
        "tcDigJB",
        "Whisky  Jim Beam (Bourbon)"
      )}">${emojiForOpt("digestif:whisky_jb")}</button>
      <div class="pill-cap">${L("tcDigJB", "Jim Beam")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="digestif:cognac"    title="${L(
        "tcDigCognac",
        "Cognac"
      )}" aria-label="${L("tcDigCognac", "Cognac")}">${emojiForOpt(
        "digestif:cognac"
      )}</button>
      <div class="pill-cap">${L("tcDigCognac", "Cognac")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="digestif:baileys"   title="${L(
        "tcDigBaileys",
        "Baileys"
      )}" aria-label="${L("tcDigBaileys", "Baileys")}">${emojiForOpt(
        "digestif:baileys"
      )}</button>
  ...
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="digIce" title="${L(
        "tcIce",
        "Ice"
      )}" aria-label="${L("tcIce", "Ice")}">${emojiForFlag("digIce")}</button>
      <div class="pill-cap">${L("tcIce", "Ice")}</div>
    </div>
  </div>`;
      ...
      box.innerHTML = `
  <div class="opt-title"> ${L("tcCocktail", "Cocktail")}</div>
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill" data-opt="cocktail:campari"       title="${L(
        "tcCampari",
        "Campari"
      )}" aria-label="${L("tcCampari", "Campari")}">${emojiForOpt(
        "cocktail:campari"
      )}</button>
      <div class="pill-cap">${L("tcCampari", "Campari")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="cocktail:bloody_mary"   title="${L(
        "tcBloody",
        "Bloody Mary"
      )}" aria-label="${L("tcBloody", "Bloody Mary")}">${emojiForOpt(
        "cocktail:bloody_mary"
      )}</button>
      <div class="pill-cap">${L("tcBloody", "Bloody Mary")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="cocktail:screwdriver"   title="${L(
        "tcScrew",
        "Screw Driver"
      )}" aria-label="${L("tcScrew", "Screw Driver")}">${emojiForOpt(
        "cocktail:screwdriver"
      )}</button>
      <div class="pill-cap">${L("tcScrew", "Screw Driver")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="cocktail:gin_tonic"     title="${L(
        "tcGinTonic",
        "Gin Tonic"
      )}" aria-label="${L("tcGinTonic", "Gin Tonic")}">${emojiForOpt(
        "cocktail:gin_tonic"
      )}</button>
      <div class="pill-cap">${L("tcGinTonic", "Gin Tonic")}</div>
    </div>
    <div class="pill-wrap">
      <button class="pill" data-opt="cocktail:cuba_libre"    title="${L(
        "tcCuba",
        "Cuba Libre"
      )}"     aria-label="${L("tcCuba", "Cuba Libre")}">${emojiForOpt(
        "cocktail:cuba_libre"
      )}</button>
  ...
  <div class="opt-title" id="campariTitle" style="display:none">${L(
    "tcCampari",
    "Campari"
  )}</div>
  <div class="pill-row"  id="campariRow"   style="display:none">
    <div class="pill-wrap">
      <button class="pill" data-opt="campariMix:orange" title="${L(
        "tcJuiceOrange",
        "Orange"
      )}" aria-label="${L("tcJuiceOrange", "Orange")}">${emojiForOpt(
        "campariMix:orange"
    ...
    <div class="pill-wrap">
      <button class="pill" data-opt="campariMix:soda"   title="${L(
        "tcSoda",
        "Soda"
      )}" aria-label="${L("tcSoda", "Soda")}">${emojiForOpt(
        "campariMix:soda"
      )}</button>
  ...
  <div class="opt-title" id="bmTitle" style="display:none">${L(
    "tcBloody",
    "Bloody Mary"
  )}</div>
  <div class="pill-row"  id="bmRow"   style="display:none">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="virginMary" title="${L(
        "tcVirgin",
        "Virgin Mary"
      )}" aria-label="${L("tcVirgin", "Virgin Mary")}">${emojiForFlag(
        "virginMary"
      )}</button>
  ...
  <div class="pill-row">
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="cocktailIce"   title="${L(
        "tcIce",
        "Ice"
      )}"     aria-label="${L("tcIce", "Ice")}">${emojiForFlag(
        "cocktailIce"
    ...
    <div class="pill-wrap">
      <button class="pill toggle" data-flag="cocktailLemon" title="${L(
        "tcLemon",
        "Lemon"
      )}" aria-label="${L("tcLemon", "Lemon")}">${emojiForFlag(
        "cocktailLemon"
    ...
    <div class="pill-wrap" style="display:none">
      <button class="pill toggle" data-flag="cocktailSP"
        ...
        "tcPepper",
        "Pepper"
      )}">${emojiForFlag("cocktailSP")}</button>
      <div class="pill-cap">${L("tcSalt", "Salt")} + ${L(
        "tcPepper",
        "Pepper"
      )}</div>
...
function focusDrinkGroup(byCat) {
  // byCat: 'chaud' | 'soft' | 'alcool'
  const keep =
    byCat === "chaud" ? "grpHot" : byCat === "soft" ? "grpSoft" : "grpAlco";
  ["grpHot", "grpSoft", "grpAlco"].forEach((id) => {
    const el = document.getElementById(id);
...
function resetDrinkUI(hard) {
  ...
  //  remise  zro sre des familles + focus: montrer Hot/Soft/Alcohol
  drinkSel.cat = "";
  drinkSel.sub = "";
  if (typeof showAllDrinkGroups === "function") showAllDrinkGroups(); //  affiche les 3 cartes
  if (typeof updateServeDrinkButtons === "function") updateServeDrinkButtons();
  applyDrinkEmojis(document.getElementById("drinkGrid"));
}
// Listeners catgorie (//)   attacher quand la modale souvre
...
function setActive(btn, scopeSel) {
  document.querySelectorAll(scopeSel).forEach((x) => {
    x.classList.remove("active");
    x.setAttribute("aria-pressed", "false");
  });
...
function updateServeDrinkButtons() {
  ...
  if (drinkSel.cat && drinkSel.sub) {
    if (L1_OK.has(drinkSel.sub)) {
      // caf / dca / caotina / sprite / tonic / champagne
      ok = true;
    } else {
  ...
  // Dsactive tout, puis n'active que le bouton de la phase
  [btnAp, btnTC, btnMeal].forEach((b) => {
...
// --- Helper : ractive toutes les pills du drink grid ---
function reactivateDrinkGrid() {
  const grid = document.getElementById("drinkGrid");
  if (!grid) return;
  grid.querySelectorAll(".pill").forEach((b) => {
    // enlve toute dsactivation rsiduelle
    b.disabled = false;
...
function ensureDrinkGridActive() {
  ...
  // Remet daplomb la grille et ses listeners (idempotent)
  try {
    reactivateDrinkGrid?.();
    initDrinkGrid?.();
...
//  Met  jour le libell du bouton boisson selon la phase
...
// === [AJOUT] Pilotage des sous-menus boissons (2 tages) ===
(function initDrinkMenus() {
  ...
  function showAlcoholSub(kind) {
    ...
    //  Toujours cacher le (i) et la bulle quand on change de sous-menu
  ...
  // init si quelque chose tait dj slectionn
  showGroup(g?.value || "");
  showAlcoholSub(a?.value || "");
  applyDrinkEmojis(document.getElementById("drinkGrid"));
})();
...
function setActiveFilterTab(filter) {
  document.querySelectorAll("#filterTabs .subtab").forEach((btn) => {
...
function updateModalStatusBadge() {
  ...
  pill.className = STATUS_PILL_CLASS[key] || STATUS_PILL_CLASS.OTHER;
...
function updateModalOccupancyControls() {
  ...
  document.querySelectorAll('input[name="m_type"]').forEach((input) => {
...
function applyI18n() {
  ...
  const laterCustomButton = document.querySelector(
  ...
  try {
    ...
    if (modalOpen && document.getElementById("drinkGrid")) {
      ...
      renderDrinkSub?.();
      renderDrinkOpts?.();
      // Montre tous les groupes (vite un tat filtr persistant)
      showAllDrinkGroups?.();
      // Scurit : r-active linteraction sur toutes les pills
      document.querySelectorAll("#drinkGrid .pill").forEach((b) => {
        b.disabled = false;
        b.style.pointerEvents = "auto";
      });
      // Met  jour ltat des boutons "Servir"
      updateServeDrinkButtons?.();
  ...
  for (const item of items) {
    ...
    btn.addEventListener("keydown", (evt) => {
      ...
      if (key === "ArrowLeft" || key === "ArrowRight") {
        ...
        if (!targetKey) return;
        const targetBtn = carousel.querySelector(`[data-seat='${targetKey}']`);
        targetBtn?.focus();
      }
    });
    carousel.appendChild(btn);
  }
  const active = carousel.querySelector('[aria-selected="true"]');
